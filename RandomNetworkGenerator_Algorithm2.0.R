...
##################Notes for SimNet Version 2.0: Blitzstein & Diaconis's Sequential Method
...
# 4. No optimization by their article but eaisier to illustrate
...
# 7. Ver 2.1 is a better rigorous version with optimization
...
EG_check <- function(DegreeDist, N){
  check_vec <- c(0)
  ...
  while (max(ResDegree)>0) {
    ...
    while (VDegree>0) {
      ...
      for (i in c(1:L)) {
        ...
        EG_ResSeq[i] <- EG_check(TestDegree,N)
      }
